...
- `snake_case` for modules, functions, variables; `PascalCase` for classes; `SCREAMING_SNAKE_CASE` for constants. Match what already exists (e.g. `coordinator.py`, `PlejdDataUpdateCoordinator`, `PLEJD_SVC_UUID`).
- Booleans: `is` / `has` / `should` / `can` prefix. Functions: verb-first (`async_set_brightness`, `decode_state_notification`, `encrypt_command`).
- Factories: `make_*` / `create_*`. Converters: `to_*`. Predicates: `is_*` / `has_*`.
- Abbreviations only when universally known here (`id`, `url`, `api`, `db`, `ble`, `gatt`, `svc`, `char` for characteristic, `addr` for BLE address). Keep protocol/cloud field names verbatim as captured (`cryptoKey`, `deviceId`, `outputAddress`) — don't snake-case wire keys.
- HA entity classes follow the platform convention: `Plejd<Platform>` (e.g. `PlejdLight`, `PlejdSwitch`, `PlejdSceneButton`).
