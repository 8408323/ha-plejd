...
- Prefer real implementations. Mock only at system boundaries (the BLE transport, the Plejd cloud HTTP API, filesystem, clock, randomness). Don't mock the crypto/codec — build real encrypted payloads with the test helpers and assert the decoded output.
